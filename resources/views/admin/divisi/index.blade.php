    ...
    <div class="x_panel">
      <div class="x_content">
        <ul class="nav nav-tabs bar_tabs" id="myTab" role="tablist">
          ...
          <li class="nav-item">
            <a class="nav-link" id="jabatan-tab" data-toggle="tab" href="#jabatan" role="tab" aria-controls="jabatan" aria-selected="false">Jabatan</a>
          </li>
        </ul>
    ...
    <tbody>
        ...
        @foreach ($divisi as $item)
            <tr>
                ...
                <td>
                    <center>
                        <a href="{{route('editdivisi_',$item->id)}}" class="btn btn-round btn-info"><span class="glyphicon glyphicon-pencil"></span></a>
...
@endpush
          ...
          <div class="tab-pane fade" id="jabatan" role="tabpanel" aria-labelledby="jabatan-tab">
           ...
           {{-- TAB JABATAN START  --}}
           <div class="x_title">
               <h2><button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target=".bs-createjabatan-modal-lg" style="margin-top: 2px;" id="tambahjabatan"><i class="fa fa-plus"></i> Tambah Jabatan</button></h2>
               <ul class="nav navbar-right panel_toolbox">
                   <li><a class="collapse-link"><i class="fa fa-chevron-up"></i></a>
                   </li>
                   <li><a class="close-link"><i class="fa fa-close"></i></a>
               ...
               <tbody>
                   @foreach ($jabatan as $item)
                       <tr>
                           ...
                           <td>{{Auth::user()->project->nama}}</td>
                           <td>{{$item->nama}}</td>
                           <td>{{$item->created_at}}</td>
                           <td>
                               <center>
                                   {{--   <a href="{{route('editdivisi',$item->id)}}" class="btn btn-round btn-info"><span class="glyphicon glyphicon-pencil"></span></a> --}}
                                   <a href="{{route('editjabatan_',$item->id)}}" class="btn btn-round btn-info"><span class="glyphicon glyphicon-pencil"></span></a>
...
<div class="modal fade bs-example-modal-jabatan" id="modal-konfirmasi-jabatan" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-jabatan">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="myModalLabel2" style="font-size: 16px;">Konfirmasi Hapus Data Posisi?</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"></span>
          </button>
        </div>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
          <button type="button" class="btn btn-danger" data-id="" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Deleting..." id="confirm-delete-jabatan_">Delete</button>
        </div>
      </div>
    </div>
</div>

@include('admin.divisi.create-jabatan')
...
$("#modal-konfirmasi-jabatan").find("#confirm-delete-jabatan_").data("id", $(this).data('id'));
$("#konfirmasi-body-jabatan").text("Hapus Data ?");
});

$('#confirm-delete-jabatan_').click(function(){
  var deleteButton = $(this);
  var id          = deleteButton.data("id");
